...
use Dizzy\Cashier\Mollie\Payment as MolliePayment;
use Dizzy\Cashier\Mollie\Customer as MollieCustomer;
...
trait Billable
{
    ...
    public function charge($amount, array $options = [])
    {
        ...
        //if (! array_key_exists('source', $options) && $this->mollie_id) {
        //    $options['customer'] = $this->mollie_id;
        ///}
        ...
        if (! array_key_exists('customerId', $options)) {
            $options['customerId'] = $this->mollie_id;
        }
        ...
        return MolliePayment::create($options);
    }
     ...
     * @param  string  $plan
     * @return \Dizzy\Cashier\SubscriptionBuilder
     */
     ...
     *
     * @param  string  $subscription
     * @return \Dizzy\Cashier\Subscription|null
     */
    ...
    /*public function invoice()
    {
        if ($this->mollie_id) {
            try {
                return MollieInvoice::create(['customer' => $this->stripe_id], $this->getStripeKey())->pay();
     ...
     *
     * @return \Dizzy\Cashier\Invoice|null
     */
    ...
    /*public function cards($parameters = [])
    {
        ...
        $stripeCards = $this->asMollieCustomer()->->all(
    ...
    /**
     * Synchronises the customer's card from Mollie back into the database.
     *
     * @return $this
     */
    /*public function updateCardFromMollie()
    {
        $customer = $this->asMollieCustomer();
    ...
    /**
     * Determine if the Mollie model is actively subscribed to one of the given plans.
     *
    ...
    public function subscribedToPlan($plans, $subscription = 'default')
    {
        ...
        foreach ((array) $plans as $plan) {
            if ($subscription->mollie_plan === $plan) {
                return true;
            }
        }
    ...
    public function onPlan($plan)
    {
        return ! is_null($this->subscriptions->first(function ($value) use ($plan) {
            return $value->mollie_plan === $plan && $value->valid();
        }));
    }
     ...
     */
    public function createAsMollieCustomer($token, array $options = [])
    {
        ...
        // Here we will create the customer instance on Mollie and store the ID of the
        // user from Mollie. This ID will correspond with the Mollie user instances
        // and allow us to retrieve users from Mollie later when we need to work.
        $customer = MollieCustomer::create($options);

        $this->mollie_id = $customer->id;
    ...
    {
        return MollieCustomer::get($this->mollie_id);
    }
